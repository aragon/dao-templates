    ...
    for (const creationStyle of creationStyles) {
        context(`> Creation through ${creationStyle} transaction`, () => {
            ...
            it('fails creating a DAO if holders and stakes don\'t match', async() => {
                ...
                if (creationStyle === 'single') {
                    try {
                        await kit.newTokenAndInstance(tokenName, tokenSymbol, aragonId, holders, stakes, neededSupport, minimumAcceptanceQuorum, votingTime)
                    } catch (err) {
                    ...
                    // create Instance
                    try {
                        await kit.newInstance(aragonId, holders, stakes, neededSupport, minimumAcceptanceQuorum, votingTime)
                    } catch (err) {
                        assert.equal(err.receipt.status, 0, "It should have thrown")
                        return
                    }
