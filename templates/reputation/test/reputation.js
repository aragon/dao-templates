...
contract('Reputation', ([_, deployer, holder1, holder2, holder3]) => {
  let daoID, reputationTemplate, dao, acl, receipt
  ...
  before('fetch reputation template', async () => {
    reputationTemplate = ReputationTemplate.at((await deployedAddresses()).address)
  })
  ...
  context('when the creation fails', () => {
    ...
    it('reverts when there was no token cached', async () => {
      await assertRevert(reputationTemplate.newInstance.request('id', HOLDERS, STAKES, { from: deployer }), 'REPUTATION_MISSING_TOKEN_CACHE')
    })
  })
