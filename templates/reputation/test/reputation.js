      ...
      context('when the creation fails', () => {
        ...
        if (creationStyle === 'single') {
          ...
          context('when there was no token created before', () => {
            it('reverts', async () => {
              await assertRevert(template, newInstanceTx(daoID, HOLDERS, STAKES, VOTING_SETTINGS, DEFAULT_FINANCE_PERIOD, true), 'REPUTATION_MISSING_TOKEN_CACHE')
            })
          })
          ...
          context('when there was a token created', () => {
            ...
            it('reverts when no holders were given', async () => {
              await assertRevert(template, newInstanceTx(daoID, [], [], VOTING_SETTINGS, DEFAULT_FINANCE_PERIOD, true), 'REPUTATION_EMPTY_HOLDERS')
            })

            it('reverts when holders and stakes length do not match', async () => {
              await assertRevert(template, newInstanceTx(daoID, [holder1], STAKES, VOTING_SETTINGS, DEFAULT_FINANCE_PERIOD, true), 'REPUTATION_BAD_HOLDERS_STAKES_LEN')
              await assertRevert(template, newInstanceTx(daoID, HOLDERS, [1e18], VOTING_SETTINGS, DEFAULT_FINANCE_PERIOD, true), 'REPUTATION_BAD_HOLDERS_STAKES_LEN')
            })
          })
        }
      })
