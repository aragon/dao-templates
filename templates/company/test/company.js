      ...
      context('when the creation fails', () => {
        ...
        if (creationStyle === 'single') {
          it('reverts when no holders were given', async () => {
            await assertRevert(template.newTokenAndInstance.request(daoID, [], [], true), 'COMPANY_EMPTY_HOLDERS')
          })

          it('reverts when holders and stakes length do not match', async () => {
            await assertRevert(template.newTokenAndInstance.request(daoID, [holder1], STAKES, true), 'COMPANY_BAD_HOLDERS_STAKES_LEN')
            await assertRevert(template.newTokenAndInstance.request(daoID, HOLDERS, [1e18], true), 'COMPANY_BAD_HOLDERS_STAKES_LEN')
          })
        } else if (creationStyle === 'separate') {
          context('when there was no token created before', () => {
            it('reverts', async () => {
              await assertRevert(template.newInstance.request(daoID, HOLDERS, STAKES, true), 'COMPANY_MISSING_TOKEN_CACHE')
            })
          })
          ...
          context('when there was a token created', () => {
            ...
            it('reverts when no holders were given', async () => {
              await assertRevert(template.newInstance.request(daoID, [], [], true), 'COMPANY_EMPTY_HOLDERS')
            })

            it('reverts when holders and stakes length do not match', async () => {
              await assertRevert(template.newInstance.request(daoID, [holder1], STAKES, true), 'COMPANY_BAD_HOLDERS_STAKES_LEN')
              await assertRevert(template.newInstance.request(daoID, HOLDERS, [1e18], true), 'COMPANY_BAD_HOLDERS_STAKES_LEN')
            })
          })
        }
      })
        ...
        const itHandlesInstanceCreationsProperly = (useAgentAsVault) => {
          ...
          before('create company entity', async () => {
            if (creationStyle === 'single') {
              ...
              instanceReceipt = await template.newInstance(daoID, HOLDERS, STAKES, useAgentAsVault, { from: owner })
            }
