...
contract('Company', ([_, deployer, holder1, holder2, holder3]) => {
  ...
  describe('newInstance', async () => {
    context('when there was no token created before', () => {
      it('reverts', async () => {
        await assertRevert(template.newInstance.request('id', HOLDERS, STAKES, { from: deployer }), 'COMPANY_MISSING_TOKEN_CACHE')
      })
    })
    ...
    context('when there was a token created', () => {
      ...
      context('when the given holders and stakes length do not match', () => {
        it('reverts', async () => {
          await assertRevert(template.newInstance.request('id', [], STAKES, { from: deployer }), 'COMPANY_BAD_HOLDERS_STAKES_LEN')
          await assertRevert(template.newInstance.request('id', HOLDERS, [], { from: deployer }), 'COMPANY_BAD_HOLDERS_STAKES_LEN')
        })
      })
