  ...
  context('when the creation fails', () => {
    context('when there was no DAO prepared', () => {
      it('reverts when trying to setup a new DAO', async () => {
        await assertRevert(template.setupInstance.request(randomId(), BENEFICIARY_KEYS, HEIRS, HEIRS_STAKE), 'TRUST_MISSING_SENDER_CACHE')
      })

      it('reverts when trying to setup a new multisig wallet', async () => {
        await assertRevert(template.setupMultiSig.request(MULTI_SIG_KEYS), 'TRUST_MISSING_SENDER_CACHE')
      })
    })
    ...
    context('when there was a DAO prepared', () => {
      ...
      it('reverts when the given beneficiary keys are not 2', async () => {
        await assertRevert(template.setupInstance.request(daoID, [beneficiaryKey1], HEIRS, HEIRS_STAKE), 'TRUST_BAD_BENEFICIARY_KEY_LENGTH')
        await assertRevert(template.setupInstance.request(daoID, [beneficiaryKey1, beneficiaryKey2, heir1], HEIRS, HEIRS_STAKE), 'TRUST_BAD_BENEFICIARY_KEY_LENGTH')
      })

      it('reverts when the given heirs do not match', async () => {
        await assertRevert(template.setupInstance.request(daoID, BENEFICIARY_KEYS, HEIRS, [66e18]), 'TRUST_BAD_HEIRS_LENGTH')
      })

      it('reverts when the given heirs stake do not represent a 66%', async () => {
        await assertRevert(template.setupInstance.request(daoID, BENEFICIARY_KEYS, HEIRS, [1e18, 1e18]), 'TRUST_INVALID_HEIRS_STAKE')
      })
      ...
      context('when there was a DAO setup', () => {
        ...
        it('reverts when then given multi sig keys are not 2', async () => {
          await assertRevert(template.setupMultiSig.request([multiSigKey1]), 'TRUST_BAD_MULTI_SIG_KEYS_LENGTH')
          await assertRevert(template.setupMultiSig.request([multiSigKey1, multiSigKey2, heir1]), 'TRUST_BAD_MULTI_SIG_KEYS_LENGTH')
        })
      })
    })
  })
