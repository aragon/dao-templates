      ...
      context('when the creation fails', () => {
        ...
        if (creationStyle === 'single') {
          it('reverts when no members were given', async () => {
            await assertRevert(template.newTokenAndInstance.request(daoID, [], true), 'MEMBERSHIP_MISSING_MEMBERS')
          })
        } else if (creationStyle === 'separate') {
          context('when there was no token created before', () => {
            it('reverts', async () => {
              await assertRevert(template.newInstance.request(daoID, MEMBERS, true), 'MEMBERSHIP_MISSING_TOKEN_CACHE')
            })
          })
          ...
          context('when there was a token created', () => {
            ...
            it('reverts when no members were given', async () => {
              await assertRevert(template.newInstance.request(daoID, [], true), 'MEMBERSHIP_MISSING_MEMBERS')
            })
          })
        }
      })
      ...
      context('when the creation succeeds', () => {
        ...
        const itHandlesInstanceCreationsProperly = (useAgentAsVault) => {
          ...
          before('create membership entity', async () => {
            if (creationStyle === 'single') {
              instanceReceipt = await template.newTokenAndInstance(daoID, MEMBERS, useAgentAsVault, { from: owner })
              tokenReceipt = instanceReceipt
            } else if (creationStyle === 'separate') {
              tokenReceipt = await template.newToken({ from: owner })
              instanceReceipt = await template.newInstance(daoID, MEMBERS, useAgentAsVault, { from: owner })
            }
